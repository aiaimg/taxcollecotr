    ...
    def get(self, request):
        ...
        try:
            ...
            if not cylindree_str:
                return JsonResponse({"success": False, "error": "Paramètre cylindree manquant"}, status=400)

            try:
                cylindree = int(cylindree_str)
            except ValueError:
                return JsonResponse({"success": False, "error": "La cylindrée doit être un nombre entier"}, status=400)

            # Obtenir les informations de conversion
            conversion_info = get_conversion_info(cylindree)

            if conversion_info["valid"]:
                return JsonResponse({"success": True, "data": conversion_info})
            else:
                return JsonResponse({"success": False, "error": conversion_info["message"]}, status=400)
