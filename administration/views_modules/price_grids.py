...
class PriceGridListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search:
            queryset = queryset.filter(
                ...
                Q(annee_fiscale__icontains=search)
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        context['fiscal_years'] = GrilleTarifaire.objects.values_list(
            'annee_fiscale', flat=True
        ).distinct().order_by('-annee_fiscale')
...
def price_grid_bulk_update(request):
    ...
    try:
        ...
        if not action or not item_ids:
            return JsonResponse({
                'success': False,
                'message': 'Missing action or items'
            }, status=400)
...
def _export_csv(queryset):
    ...
    response['Content-Disposition'] = f'attachment; filename="price_grids_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
...
def _export_json(queryset):
    ...
    response['Content-Disposition'] = f'attachment; filename="price_grids_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json"'
...
def price_grid_report(request):
    ...
    if report_type == 'distribution':
        ...
        by_energy = queryset.values('source_energie').annotate(
            count=Count('id'),
        ...
        by_year = queryset.values('annee_fiscale').annotate(
            count=Count('id'),
            avg_amount=Avg('montant_ariary')
        ...
        context = {
            ...
            'fiscal_years': GrilleTarifaire.objects.values_list(
                'annee_fiscale', flat=True
            ).distinct().order_by('-annee_fiscale')
        }
