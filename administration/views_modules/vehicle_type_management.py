...
class VehicleTypeManagementListView(AdminRequiredMixin, ListView):
    ...
    model = VehicleType
    template_name = "administration/vehicle_type_management/list.html"
    context_object_name = "vehicle_types"
    paginate_by = 20

    def get_queryset(self):
        queryset = VehicleType.objects.annotate(vehicle_count=Count("vehicules")).order_by("ordre_affichage", "nom")
...
class VehicleTypeManagementDetailView(AdminRequiredMixin, DetailView):
    ...
    model = VehicleType
    template_name = "administration/vehicle_type_management/detail.html"
...
class VehicleTypeManagementCreateView(AdminRequiredMixin, CreateView):
    ...
    model = VehicleType
    template_name = "administration/vehicle_type_management/form.html"
...
class VehicleTypeManagementUpdateView(AdminRequiredMixin, UpdateView):
    ...
    def form_invalid(self, form):
        messages.error(
            self.request, "Erreur lors de la modification du type de véhicule. Veuillez vérifier les champs."
        )
        return super().form_invalid(form)
...
class VehicleTypeManagementDeleteView(AdminRequiredMixin, DeleteView):
    ...
    model = VehicleType
    template_name = "administration/vehicle_type_management/confirm_delete.html"
    success_url = reverse_lazy("administration:vehicle_type_list")
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if vehicle_count > 0:
            messages.error(
                request,
                f"Impossible de supprimer le type '{vehicle_type.nom}' car {vehicle_count} véhicule(s) l'utilisent encore.",
            )
...
def vehicle_type_bulk_import(request):
    ...
    if request.method == "GET":
        return render(request, "administration/vehicle_type_management/bulk_import.html")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if errors:
                messages.warning(
                    request,
                    f"Import terminé avec {len(errors)} erreurs. {created_count} créés, {updated_count} mis à jour.",
                )
                for error in errors[:10]:  # Show first 10 errors
                    messages.error(request, error)
            else:
                messages.success(
                    request, f"Import réussi: {created_count} types de véhicules créés, {updated_count} mis à jour."
                )
