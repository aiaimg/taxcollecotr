...
class StripeConfigManageView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        ...
        active_env = 'development' if (dev_cfg.is_active and not prod_cfg.is_active) else (
            'production' if (prod_cfg.is_active and not dev_cfg.is_active) else None
        )
    ...
    def post(self, request):
        ...
        if action == 'save_development':
            ...
            return render(request, self.template_name, {
                ...
                'dev_cfg': dev_cfg,
                'prod_cfg': prod_cfg,
                'active_env': 'development' if dev_cfg.is_active else ('production' if prod_cfg.is_active else None),
            })
