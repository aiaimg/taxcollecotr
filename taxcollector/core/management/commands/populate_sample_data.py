...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self.stdout.write(
                self.style.SUCCESS(f'Created tax grid for year {current_year}')
            )
        else:
            self.stdout.write(
                self.style.WARNING(f'Tax grid for year {current_year} already exists')
            )
        ...
        if created_count > 0:
            self.stdout.write(
                self.style.SUCCESS(f'Successfully created {created_count} tariff entries')
            )
        else:
            self.stdout.write(
                self.style.WARNING('All tariff entries already exist')
            )
